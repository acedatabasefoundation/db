...
export function validateSchema (schema) {
  ...
  for (const nodeName in schema.nodes) {
    ...
    if (nodeName.startsWith('Ace')) throw AceError('schema__invalidNodeStart', `Please ensure each node name does not start with "Ace" when updating the schema, this is not happening yet for the node name: ${ nodeName }. This rule is applied to avoid types name clashing.`, { nodeName })
  ...
  for (const relationshipName of relationshipNameArray) {
    ...
    if (relationship?.is !== 'OneToOne' && relationship?.is !== 'ManyToMany' && relationship?.is !== 'OneToMany') throw AceError('schema__invalidRelationship_Id', `Please ensure each schema.relationships[relationshipName].is is OneToOne, OneToMany or ManyToMany, this is not happenin yet for the relationship name: ${ relationshipName }`, _errorData)
    if (relationshipName.startsWith('Ace')) throw AceError('schema__invalidRelationshipStart', `Please ensure each relationship name does not start with "Ace" when updating the schema, this is not happening yet for the relationship name: ${ relationshipName }. This rule is applied to avoid types name clashing.`, { relationshipName })
