...
import { ENUM_ID_PREFIX, ADD_NOW_DATE, getNow, getRelationshipProp, getRelationship_IdsKey, getUniqueIndexKey } from '../../util/variables.js'
...
export async function inupRelationship (jwks, reqItem) {
  if (Memory.txn.schema?.relationships?.[reqItem?.how?.relationship]) {
    ...
    if (!schemaRelationship) throw AceError('unknownRelationshipName', `Please ensure each relationship name is defined in the schema, this is not happening yet for reqItem.how.relationship: ${ reqItem.how.relationship }`, { reqItem })
    ...
    if (reqItem.do === 'RelationshipUpdate') {
      ...
      if (!graphNode) throw AceError('invalidUpdateId', `Please ensure reqItem.how.props._id is an _id defined in your graph, this is not happening yet for reqItem.how.props._id: ${ reqItem.how.props._id }`, { reqItem })
      if (graphNode.relationship !== reqItem.how.relationship) throw AceError('invalidUpdateRelationshipName', `Please ensure graphNode.relationship is equal to reqItem.how.relationship, this is not happening yet for graphNode.relationship: ${ graphNode.relationship },  and reqItem.how.relationship: ${ reqItem.how.relationship }`, { reqItem, graphNode })
...
async function inupRelationshipPut (jwks, reqItem, schemaRelationship) {
  ...
  for (const propName in props) {
    if (typeof props[propName] === 'string' && props[propName].startsWith(ENUM_ID_PREFIX)) overwriteIds(props, propName)
      ...
      if (!schemaProp) throw AceError('invalidRelationshipProp', `Please ensure when mutating relationships the relationship name and prop name exist in the schema. This is not happening yet for the relationship name: "${ reqItem.how.relationship }" and the prop name: "${ propName }"`, { reqItem })
      ...
      if (schemaProp.options.dataType === 'iso' && props[propName] === ADD_NOW_DATE) props[propName] = getNow() // populate now timestamp
  ...
  /** @type { (string | number)[] } */
  const relationship_IdsArray = await getOne(relationship_IdsKey) || []
  const relationship_IdsSet = new Set(relationship_IdsArray)

  relationship_IdsSet.add(props._id) // add relationship id to relationship index

  if (reqItem.do === 'RelationshipInsert') write('insert', relationship_IdsKey, [ ...relationship_IdsSet ])
  else write('update', relationship_IdsKey, [ ...relationship_IdsSet ])
}
